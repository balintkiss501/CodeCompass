...
import parser.util.Hashing;
import parser.util.HtmlBuilder;
...
public class AstVisitor extends org.eclipse.jdt.core.dom.ASTVisitor {
  ...
  private Map<String, JavaFunction> functions = new HashMap<>();  // The functions meet by the visitor. Map mangled name to function.
  private Map<String, JavaType> types = new HashMap<>();          // The types meet by the visitor. Map qualified name to type.
  private List<ClassInstanceCreation> anonymusClassInstantiations = new LinkedList<>();
  ...
  public AstVisitor(
          ...
          final boolean createBuildAction) throws ParseException {
    this.em = em;
    this.cu = cu;
    ...
    if (filename.endsWith(".java")) {
      visitSource(parentPath, filename, src, timestamp, buildId, createBuildAction);
    } else if (filename.endsWith(".class")) {
      visitDecompiledSource(parentPath, filename, src, timestamp);
    }
  }
  ...
  private void visitSource(
          ...
          final String buildId,
          final boolean createBuildAction) throws ParseException {
    File fparent = Query.getOrCreateDirectory(em, project, parentPath, timestamp);

    // Persist source and code content
    if ((file = Query.getFileByName(em, parentPath + "/" + filename)) == null) {
      FileContent content = new FileContent();
      content.setContent(new String(src));
      content.setHash(Hashing.computeContentHash(src));
      em.persist(content);
    ...
    // Persist compiled .class
    if ((classfile = Query.getFileByName(em, parentPath + "/" + classfilename)) == null) {
      classfile = new File();
      classfile.setPath(parentPath + "/" + classfilename);
      classfile.setFilename(classfilename);
      classfile.setType(File.JAVA_CLASS);
    ...
    if (no_errors) {
      file.setParseStatus(File.PS_FULLY_PARSED);
    } else {
      file.setParseStatus(File.PS_PARTIALLY_PARSED);
    }
  }

  private void visitDecompiledSource(
          final String parentPath,
          final String filename,
          final char[] src,
          final long timestamp) throws ParseException {
    File fparent = Query.getOrCreateDirectory(em, project, parentPath, timestamp);
    ...
    if ((file = Query.getFileByName(em, parentPath + "/" + filename)) == null) {
      ...
      file.setContent(content);
      file.setTimestamp(timestamp);
      file.setType(File.JAVA_SOURCE);   // FIXME: It should be JAVA_CLASS, but GUI won't highlight code that way
      em.persist(file);
    }
  ...
  private void error(final ASTNode node, final String message) {
  ...
  private void error(
          ...
          final boolean trace) {
    if (File.PS_FULLY_PARSED == file.getParseStatus()) {
      file.setParseStatus(File.PS_PARTIALLY_PARSED);
    }
  ...
  private static int _calcNewNodeStart(final int start, final ASTNode node) {
  ...
  private JavaAstNode createOrGetDummyNode() {
    final long dummyNodeId = Hashing.permaAstId("dummy", "dummy");
    JavaAstNode node = em.find(JavaAstNode.class, dummyNodeId);
  ...
  public void pushVar(final String name, final JavaAstNode node) {
    trace("pushVar: " + name);

    frame.addVariable(name, node);
  }

  private JavaAstNode findVar(final String name) {
    return frame.findVariable(name);
  }

  private int translateModifiers(final int mods) {
    int m = 0;
  ...
  public String getQualifedName(final String var) {
  ...
  private JavaAstNode createFakeAstNode(final JavaType type, final JavaType genericType) {
    String mangledName = type.getQualifiedName();
    String val = "type " + type.getName();
    final long id = Hashing.permaAstId(val, mangledName, file, 0, 1);

    JavaAstNode n = em.find(JavaAstNode.class, id);
    ...
    if (null == n) {
      ...
      n.setMangledNameHash(Hashing.getNameHash(mangledName));
  ...
  private JavaAstNode createFakeAstNode(final JavaFunction func, final JavaFunction genericFunc) {
  ...
  private JavaAstNode createAstNode(
          ASTNode node,
          String val,
          JavaAstNode.AstType at,
  ...
  private JavaAstNode createAstNode(
    ...
    long id = Hashing.permaAstId(val, mangledName, file, pi.getStart(), pi.getEnd());
  ...
  private JavaAstNode createVariable(
          ASTNode node,
          JavaType type,
          String name,
          int mods,
  ...
  private JavaAstNode createVariable(
          ...
          JavaFunction func,
          Javadoc doc) {
    JavaAstNode n = createAstNode(
            node,
            "var " + name,
            JavaAstNode.AstType.Declaration,
            JavaAstNode.SymbolType.Variable,
            getQualifedName(name));
  ...
  private JavaFunction createFunction(
    ...
    JavaAstNode new_node = createAstNode(
            node,
            "func " + name,
            JavaAstNode.AstType.Definition,
            JavaAstNode.SymbolType.Function,
            mangledName,
            node_offs);
  ...
  private JavaMember registerField(
          final JavaAstNode node,
  ...
  public JavaVariable registerLocal(
          final JavaAstNode n,
          final JavaType type,
          final String name) {
  ...
  public JavaVariable registerParam(
          final JavaAstNode n,
          final JavaType type,
          final String name,
          final JavaFunction func) {
    JavaVariable var = new JavaVariable(type, n, name, getQualifedName(name));
    var.setMangledNameHash(n.getMangledNameHash());
  ...
  public JavaFunction registerFunction(
    ...
    func.setName(name);
    // func.setMangledNameHash(Common.getNameHash(mangledName));
  ...
  private JavaDocComment createJavadoc(final long mangledNameHash, final Javadoc doc) {
  ...
  private void processAnnotations(final JavaAstNode astnode, final List mods) {
    if (null == mods) {
      return;
    }
  ...
  private JavaFunction loadOrCreateFunction(final IMethodBinding binding, final String mangledName) {
    ...
    // JavaFunction func = Common.findFunction(fs, mangledName);
    JavaFunction func = em.find(JavaFunction.class, Hashing.getNameHash(mangledName));
    ...
    if (null == func) {
      ...
      func.setMangledName(mangledName);
      func.setMangledNameHash(Hashing.getNameHash(mangledName));
  ...
  private JavaFunction functionLookUp(final IMethodBinding methbind, final String mangledName) {
    return functionLookUp(methbind, mangledName, false);
  }

  private JavaFunction functionLookUp(
          final IMethodBinding methbind,
          final String mangledName,
          final boolean eraseParams) {
...
//		String qualName = nex.removeGenericParams().getFullName();
  ...
  private JavaType loadOrCreateType(final ITypeBinding binding, final String qualName) {
    // TypedQuery<JavaType> ts = em.createQuery("SELECT t FROM JavaType t WHERE t.qualifiedName like \"" + qualName + "\"", JavaType.class);
    // JavaType type = Common.findType(ts, qualName);
    JavaType type = em.find(JavaType.class, Hashing.getNameHash(qualName));
    ...
    if (null == type) {
      ...
      type.setMangledNameHash(Hashing.getNameHash(qualName));
  ...
  private JavaType typeLookUp(final ITypeBinding binding) {
    return typeLookUp(binding, false);
  }

  private JavaType typeLookUp(final ITypeBinding binding, final boolean eraseParams) {
  ...
  private JavaAstNode.AstType getUsageMode(ASTNode node, String typeName) {
    ASTNode child = node;
...
//
  ...
  public boolean visit(ImportDeclaration node) {
    ...
    JavaAstNode an = createAstNode(
            node,
            "import " + name,
            JavaAstNode.AstType.Declaration,
            JavaAstNode.SymbolType.Import,
            name);
  ...
  private void handleImplicitImport(
          JavaAstNode an,
          String mangledName,
          boolean isFunction) {
  ...
  private boolean handleVisitTypeDeclaration(
          ASTNode node,
          String nodeName,
  ...
  public boolean visit(EnumConstantDeclaration node) {
    ...
    JavaAstNode an = createAstNode(
            ...
            "enumval " + name,
            JavaAstNode.AstType.Declaration,
            JavaAstNode.SymbolType.EnumConstant,
            mangledName);
  ...
  public boolean visit(SingleVariableDeclaration node) {
    ...
    if (node.getParent() instanceof EnhancedForStatement) {
      ...
      if (null != binding) {
        ...
        JavaAstNode an2 = createAstNode(
                node,
                "typeloc " + id,
                JavaAstNode.AstType.TypeLocation,
                JavaAstNode.SymbolType.Type,
                mangledName);
        an2.setMangledName(mangledName);
        an2.setMangledNameHash(Hashing.getNameHash(mangledName));
  ...
  private boolean processQualifiedName(SimpleName name, QualifiedName qualName) {
    ...
    if (null == bind) {
      ...
      JavaAstNode an = createAstNode(
              name,
              "usage " + mangledName,
              getUsageMode(qualName, typeName),
              JavaAstNode.SymbolType.Variable,
              mangledName);
      an.setMangledName(mangledName);
      an.setMangledNameHash(Hashing.getNameHash(mangledName));
  ...
  private void processFieldAccess(SimpleName name, FieldAccess fieldAccess) {
    ...
    if (null == fbind || null == tbind) {
      ...
      JavaAstNode an = createAstNode(
              name,
              "usage " + id,
              getUsageMode(fieldAccess, typeName),
              JavaAstNode.SymbolType.Variable,
              mangledName);
      an.setMangledName(mangledName);
      an.setMangledNameHash(Hashing.getNameHash(mangledName));
  ...
  public boolean visit(SimpleType type) {
    if (null != type.getParent() && ((type.getParent() instanceof TypeDeclaration) || (type.getParent() instanceof EnumDeclaration))) {
      ...
      JavaAstNode an = createAstNode(
              type,
              "typeloc " + id,
              JavaAstNode.AstType.TypeLocation, JavaAstNode.SymbolType.Type, mangledName);
      an.setMangledName(mangledName);
      an.setMangledNameHash(Hashing.getNameHash(mangledName));
